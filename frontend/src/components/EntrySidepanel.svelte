<script lang="ts">
  ...
  //   onMount(async () => {
  //     entry.tasks = await getJsonAuth(
  //       "https://localhost:8001/api/Task",
  //       $auth.tokenStr
  //     );
  //     entry = entry;
  //   });
  ...
  async function removeTask(id: number) {
    const response = await sendJsonAuth(
      "https://localhost:8001/api/Task",
      "DELETE",
      $auth.tokenStr,
      {
    ...
    {#each entry.tasks.filter((t) => !t.completed) as todo}
      <li class="flex items-center space-x-2 my-2">
        <input
          type="checkbox"
          id={todo.text}
          on:change={() => completeTask(todo.id)}
        />
        <label for={todo.text}>{todo.text}</label>
        <button
          on:click={() => removeTask(todo.id)}
          class="px-4 py-2 font-bold rounded bg-red-500 text-white">x</button
        >
      </li>
    {/each}
  </ul>
